...
def make_fh_group_detail(df, period):
...
def make_fhgd_consecutive(df, first_year, period):
  FHG = 'Consecutive'   
  ...
  for year in years:
    _df['FY'+str(year)] = [get_consecutive(x, year, period) for x in _df['AllYears'].values.tolist()]

  # format results 
  return wrap_up_fhgd(df, _df, FHG, years)


def make_fhgd_lapsed(df, first_year, period):
  FHG = 'Lapsed'  
  _df = df.drop_duplicates(['DonorID', 'GiftYear']) 
  ...
  years = [x for x in range(first_year, first_year+5)]
  for year in years:
    _df['FY'+str(year)] = [get_lapsed(x, year) for x in _df['AllYears'].values.tolist()]

  # format results 
  return wrap_up_fhgd(df, _df, FHG, years)
  

def make_fhgd_reinstated(df, first_year, period):
  FHG = 'Reinstated'   
  _df = df.drop_duplicates(['DonorID', 'GiftYear']) 
  ...
  years = [x for x in range(first_year, first_year+5)]
  for year in years:
    _df['FY'+str(year)] = [get_reinstated(x, year, period) for x in _df['AllYears'].values.tolist()]
...
#     os.makedirs(os.path.join(filemap.CURATED, subset_dir, period), exist_ok=True)
    ...
    prefix = "%s/%s/%s/" % (subset_dir, period, str(year))
...
def calc_fh(df, period, year, FYM, last_gift_date):
  ...
  # FH Group Detail
  _df = make_fh_group_detail(_df, period)
