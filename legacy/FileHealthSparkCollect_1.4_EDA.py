...
# mask = df['DonorID'] == 10004807 #10291384
# df = df.filter(mask)
# df.filter(mask).toPandas().to_csv(os.path.join(filemap.CURATED, 'DAV_Donor10004807.csv'), index=False)
...
_df = _df.join(grouped, on='DonorID', how='left').dropDuplicates(['DonorID']).select(['DonorID', 'AllYears'])
...
# cast 'ConsecutiveValue' to string
c = 'ConsecutiveValue'
_df = _df.withColumn(c, _df[c].cast(StringType()))
...
# cast 'ReinstatedValue' to string
c = 'ReinstatedValue'
_df = _df.withColumn(c, _df[c].cast(StringType()))
...
# mask = (df['DonorID'] == 10004807) & (df['ReportPeriodYear'] == '2021')
# df.filter(mask).show(truncate=False)
...
# # and fill nulls with empty string
# df = df.withColumn(feature, F.when(F.col(feature).isNull(), '').otherwise(F.col(feature)))

# # concatenate columns to get FHGroupDetails
# feature = 'FHGroupDetail'
# col_list = ['FHGroup','FHGD_Value']
# df = df.withColumn(feature,F.concat(*col_list))
...
# # TODO Extract mapping dictionary
# col = 'FHGroupDetail'
# _ = df.select(F.collect_set(col).alias(col)).first()[col]
# all_maps[col] = {x: i for i,x in enumerate(sorted(_))}
# all_maps
...
# mask = _df['DonorID'] == 10004807
# _df.filter(mask).show(n=10)
