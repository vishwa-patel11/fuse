...
consec_udf = F.udf(lambda x,t,p: get_consecutive(x, t, p), StringType())
years = [x for x in range(FIRST_YEAR, FIRST_YEAR+5)]
for year in years:
  print('year: ', year)
  _df = _df.withColumn('FY'+str(year), consec_udf('AllYears', F.lit(year), F.lit(period_view)))
  
  
mask = _df['FHGroup'] == 'Consecutive Giver'
cols = ['DonorID', 'AllYears', 'FY2017', 'FY2018', 'FY2019', 'FY2020', 'FY2021']
_df.filter(mask).select(cols).show(n=30)
...
# apply consecutive fuction to list
reinstated_udf = F.udf(lambda x,t,p: get_reinstated(x,t,p), StringType())
years = [x for x in range(FIRST_YEAR, FIRST_YEAR+5)]
for year in years:
  print('year: ', year)
  _df = _df.withColumn('FY'+str(year), reinstated_udf(F.col('AllYears'), F.lit(year), F.lit(period_view)))
  
mask = _df['FHGroup'] == 'Reinstated Last Year'
cols = ['DonorID', 'AllYears', 'FY2017', 'FY2018', 'FY2019', 'FY2020', 'FY2021']
_df.filter(mask).select(cols).show(n=30)
...
col = 'FHGroupDetail'
mask = df['FHGroup'] == 0
_ = df.filter(mask).select(F.collect_set(col).alias(col)).first()[col]
_
