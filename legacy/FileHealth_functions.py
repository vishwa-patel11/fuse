...
def process_df(df, i, period, dec, runner, prefix):

  this_year = datetime.now().year
  years = [i for i in range(this_year-5, this_year)]
...
#     print(period, year)
    ...
    # Add inner dataframe
    dfs.append(_df[FH_COLS])

  processed = pd.concat(dfs)
  processed = assert_declarations(processed, dec, runner, view=True)    
  fi = prefix + str(i) + ".parquet"
  processed.to_parquet(CURATED + fi, index=False) 
...
def main(df, n, fh_period, filemap, runner, dataset, check=True):
  try:
    ...
    for i in range(n):
      print('iteration: ', i)
      start = time.time()
      process_df(df_list[i], i, fh_period, dec, runner, prefix)
...
def main_parallel(df, n, fh_period, filemap, runner, dataset, check=True):
  try:
    split_donor_sets = generate_sets_to_split(df, n, check_len=check)
    ...
    run_in_parallel = lambda x: process_df(x[0], x[1], fh_period, dec, runner, prefix)
...
def main_parallel_b(df, n, fh_period, filemap, runner, dataset, check=True):
  try:
    split_donor_sets = generate_sets_to_split(df, n, check_len=check)
    ...
    with Executor() as executor:
      for i in range(n):
        ...
        _ = executor.submit(process_df, df_list[i], i, fh_period, dec, runner, prefix)
